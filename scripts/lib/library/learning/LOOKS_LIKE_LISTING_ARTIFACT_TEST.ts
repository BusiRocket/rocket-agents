...
void test("a natural long tail repeating small counts is real usage", () => {
  ...
  const counts: Record<string, number> = { brain: 55, "llm-wiki": 31, "brp-docs": 24 }
...
void test("a catalogue cluster near the top still reads as an artifact", () => {
  ...
  for (let index = 0; index < 17; index++) counts[`catalogued-${String(index)}`] = 892
  for (let index = 0; index < 20; index++) counts[`real-${String(index)}`] = index + 2
