...
void test("a live server is captured with every target that carries it", () => {
  ...
  state.byTarget["claude-personal"].serena = { command: "serena", args: ["start-mcp-server"] }
  state.byTarget.cursor.serena = { command: "serena", args: ["start-mcp-server"] }

  const capture = captureMcpManifest({ state, declared: createDeclaredContext7Manifest() })
