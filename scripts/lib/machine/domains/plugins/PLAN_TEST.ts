...
void test("an undeclared installed plugin is planned for removal", () => {
  ...
  assert.deepEqual(changes, [{ operation: "remove", id: "alpha@official", detail: "1.0.0" }])
})
...
void test("a declared plugin that is not installed is planned for install", () => {
  ...
  assert.deepEqual(changes, [{ operation: "install", id: "alpha@official", detail: "1.0.0" }])
})
...
void test("a version drift is planned as a pin with both versions named", () => {
  ...
  assert.deepEqual(changes, [{ operation: "pin", id: "alpha@official", detail: "1.0.0 -> 2.0.0" }])
})
