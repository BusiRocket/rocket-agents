...
void test("enablement targets the profile's config directory", async () => {
  ...
  assert.equal(result.applied.length, 2)
  assert.deepEqual(calls.at(0)?.env, { CLAUDE_CONFIG_DIR: join("/home/u", ".claude-favish") })
  assert.deepEqual(calls.at(1)?.env, { CLAUDE_CONFIG_DIR: join("/home/u", ".claude") })
  assert.deepEqual(calls.at(1)?.argv, [
    "claude",
    "plugin",
    "disable",
...
void test("a pin is reported as manual, never executed", async () => {
  ...
  const result = await apply({
    changes: [{ operation: "pin", id: "caveman@busirocket", detail: "1.0.0 -> 1.2.0" }],
    paths: createPluginsPaths("/home/u"),
    run,
  })
...
void test("a failing command lands in failed with its output", async () => {
  ...
  const result = await apply({
    changes: [{ operation: "install", id: "caveman@busirocket", detail: "1.0.0" }],
    paths: createPluginsPaths("/home/u"),
    run,
  })
