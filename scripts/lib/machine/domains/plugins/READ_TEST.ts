...
void test("marketplaces are read with a flattened source", async () => {
  ...
  assert.deepEqual(await readMarketplaces(join(home, "known_marketplaces.json")), [
...
void test("a missing marketplace file reads as empty instead of throwing", async () => {
  assert.deepEqual(await readMarketplaces("/nonexistent/known_marketplaces.json"), [])
})
...
void test("installed plugins keep version, scope and commit sha", async () => {
  ...
  assert.deepEqual(await readInstalledPlugins(join(home, "installed_plugins.json")), [
    {
      id: "alpha@official",
      scope: "user",
      version: "1.0.0",
...
void test("a corrupt installed file reads as empty instead of throwing", async () => {
  const home = await createTempPluginsHome({ "installed_plugins.json": "{ not json" })
...
void test("settings without enabledPlugins read as empty", async () => {
  const home = await createTempPluginsHome({ "settings.json": JSON.stringify({ model: "opus" }) })
