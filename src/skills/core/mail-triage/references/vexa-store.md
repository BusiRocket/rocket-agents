...
| `Schema skew: database is at version N ... binary supports N-1`             | The app migrated the store past this binary. Reads fail too.                                                       | Rebuild the CLI. Do **not** migrate the store forward to suit the CLI while the app is running - that breaks the app instead. |
